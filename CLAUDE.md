...
**Current Version:** v11.9.0 - MINOR release. The headline is a dependency bump that closes two high-severity transformers advisories with no fix anywhere in the 4.x line — GHSA-29pf-2h5f-8g72 and GHSA-fgcw-684q-jj6r, both fixed only in 5.x — so the `[ml]` and `[nli]` extras now pin `transformers>=5.5.0,<6.0.0` (#305). Verifying that move needed a CI job that actually installs the ml/nli extras, since transformers had never been imported in CI before (#303), and that job immediately turned up a real bug: a *disabled* quality system still called `_ensure_initialized()` before checking `config.enabled`, so `MCP_MEMORY_QUALITY_ENABLED=false` still paid for a full `torch.onnx.export` of DeBERTa on its first call — found when it blew pytest's 120s timeout in CI (#316, closes #315). Smaller fixes riding along: `onnx_ranker` now lets `huggingface_hub` resolve its own cache instead of building paths by hand (#307, closes #304), a setuptools bound that broke milvus-lite import is corrected (#300), and `onnxscript` moved into the `[ml]` extra so the quality export actually runs from a plain install (#310). See [CHANGELOG.md](CHANGELOG.md) for details. (Issue/PR numbers refer to Codeberg.)
