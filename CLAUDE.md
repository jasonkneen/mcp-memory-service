...
**Current Version:** v11.8.3 - PATCH release, security and correctness. Three fixes carry it, and all three were wrong in a way that looked fine. The MCP SDK bump to 1.29.0 closes CVE-2026-52869, which is genuinely reachable here: HTTP transports served session requests without verifying the authenticated principal, and both affected transports are wired up (#277). `memory launch` never passed `--ssl-certfile`/`--ssl-keyfile` to uvicorn, so a deployment configured for HTTPS came back up on plain HTTP after every restart while `memory info` reported `http://` in agreement; unusable TLS now fails the launch instead of falling back, and the launched scheme is recorded in the PID file (#279). The dashboard's SSE query-string auth wrote the API key into the access log in cleartext, which TLS cannot protect; sensitive query parameters are now redacted (#285). Also here: a batch of log-injection sanitisation (#268, #270, #271) and an LM Studio compat patch that had silently stopped applying (#284). See [CHANGELOG.md](CHANGELOG.md) for details. (Issue/PR numbers refer to Codeberg.)
