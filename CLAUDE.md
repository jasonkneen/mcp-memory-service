...
**Current Version:** v11.8.1 - PATCH release: eight fixes on top of v11.8.0, six from timkjr. The two worth knowing before you upgrade: OAuth `iss` validation is now exact against an rstripped `MCP_OAUTH_ISSUER`, so tokens minted with a trailing-slash issuer need to be reissued (#239); and truthy parsing for `MCP_HTTPS_ENABLED` is now standardized into the repo-root `run_server.py` the Docker image actually runs, so `MCP_HTTPS_ENABLED=1`/`yes`/`on`/`enabled` now starts HTTPS there instead of silently falling through to HTTP (#231). Also: sqlite-vec CLI commands honor the configured embedding model (#232), `validate_imports.sh` resolves the venv Python instead of a bare `python3` (#230), `_is_https_enabled()` accepts `on`/`enabled` (#238), consolidation no longer compounds its own association records into meta-association inference and reports real run counters from `health_check` (#241, #242), and Milvus filter expressions escape backslashes instead of letting a trailing one swallow the closing quote (#244); see [CHANGELOG.md](CHANGELOG.md) for details. (Issue/PR numbers refer to Codeberg.)
