...
**Current Version:** v11.7.0 - MINOR release, in substance a security release: three unconditional TLS certificate-verification bypasses (claude-hooks #198, the opencode plugin #210, `examples/http-mcp-bridge.js`) are now gated behind an explicit `allowSelfSignedCerts` opt-in, and a real credential committed in the bundled hooks config is gone (#197/#200) with a CI gate to keep it that way. Also: configurable ONNX model directory (#171/#209), the Milvus backend's `store()` signature brought in line with the rest (#148/#133), the consolidation merge action returning the real content hash (#208/#112), and the Claude Code plugin manifest at 1.0.3; see [CHANGELOG.md](CHANGELOG.md) for details. (Issue/PR numbers refer to Codeberg.)
