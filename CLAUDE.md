...
**Current Version:** v11.8.4 - PATCH release. The headline fix is a hybrid-deployment problem, not a feature: Cloudflare's D1 free-tier daily read limit (5 million rows) starts being enforced on 1 September 2026, and the background sync loop was on pace to burn roughly 19 million rows a day, because it called `get_stats()` twice per cycle and that call scanned the memories table four times over. Fixed with a `SELECT 1` health probe, capacity monitoring moved onto its own hourly cadence (`MCP_HYBRID_CAPACITY_CHECK_INTERVAL`), a single-pass `get_stats()`, and a tombstone purge that now reaches the Cloudflare secondary (#289). Three smaller correctness fixes ride along: health reporting named the wrong embedding model when an external endpoint was configured (#290, closes #254), a hash-fallback refusal message reported memories plus embedding rows as a memory count (#290, closes #228), LM Studio startup diagnostics were landing on the stdio JSON-RPC channel (#287, closes #275), and `memory status --storage-backend hybrid` raised outright (#287, closes #233). See [CHANGELOG.md](CHANGELOG.md) for details. (Issue/PR numbers refer to Codeberg.)
